...
class AnubisConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_offer(self, offer):
        ...
        try:
            ...
            # Fetch camera URLs from the database using the CameraDetails model asynchronously
            updated_camera_urls = await sync_to_async(list)(CameraDetails.objects.values_list('url', flat=True))

            if not updated_camera_urls:
                logging.error("No camera URLs found in the updated database")
                return

            # Identify the old camera URLs that need to be stopped
            old_camera_urls = set(self.peer_connections.keys()) - set(updated_camera_urls)

            # Stop and remove old connections
            for old_url in old_camera_urls:
                try:
                    old_data = self.peer_connections.pop(old_url)
                    old_data["track"].stop()  # Stop the VideoStream associated with the old camera
                    await old_data["peer_connection"].close()  # Close the RTCPeerConnection
                    logging.info(f"Closed old connection for camera {old_url}")
                except Exception as e:
                    logging.error(f"Error closing old connection for camera {old_url}: {e}")

            # Create separate VideoStreams and RTCPeerConnections for each updated camera URL
            for camera_id, url in enumerate(updated_camera_urls, start=1):
                try:
            ...
            # Set remote description with the received offer for each updated camera
